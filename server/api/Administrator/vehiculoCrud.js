document.addEventListener("DOMContentLoaded", function () {
  ...
  async function fetchUsuarios() {
    try {
      ...
      editPropietarioSelect.innerHTML =
        '<option value="">Seleccione el propietario del vehículo</option>';
  ...
  document
    .getElementById("addVehiculoForm")
    .addEventListener("submit", async function (event) {
      ...
      const { data, error } = await supabase
        .from("Vehiculos")
        .insert([{ marca, color, placa, tipo, duenoID, cat7600, activo }]);
  ...
  document.addEventListener("click", function (event) {
    if (event.target.closest(".edit")) {
      ...
      document.getElementById("editVehiculoForm").dataset.idVehiculo =
        idVehiculo;
    }
    ...
    if (event.target.closest(".delete")) {
      ...
      const idVehiculo = button.getAttribute("data-id");
      document.getElementById("deleteVehiculoForm").dataset.idVehiculo =
        idVehiculo;
    }
  });

  document
    .getElementById("editVehiculoForm")
    .addEventListener("submit", async function (event) {
      event.preventDefault();
  ...
  document
    .getElementById("deleteVehiculoForm")
    .addEventListener("submit", async function (event) {
      event.preventDefault();
